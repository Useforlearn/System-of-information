<template>
  <div class="main-content">
    <div>
      <el-row :gutter="10">
        <el-col :span="6" v-for="item in medicalExamList" :key="item.id">
          <div style="margin-bottom: 10px; cursor: pointer" @click="$router.push('/front/medicalExamDetail?id=' + item.id)">
...
export default {
  ...
  data() {
    return {
      ...
      pageSize: 8,
      total: 0,
      name: '' // 这里 name 可能需要根据实际情况从父组件或其他地方获取，暂时设为空字符串
    }
  },
  created() {
    this.load(1);
  ...
  methods: {
    ...
    load(pageNum) {  // 分页查询
      ...
      this.$request.get('/medicalExam/selectPage', {
        ...
        this.medicalExamList = res.data?.list
